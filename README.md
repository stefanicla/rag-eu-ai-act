...
The corpus covers the full AI Act (prohibited practices, high-risk rules, GPAI duties, fines, transparency, Annex III, dates). Paste any of these into the chat:
...
1. The question is embedded with the same model used at ingest. The name is pinned in `.env` because a different model reads a different vector space.
...
* CPU answers stream at a few tokens per second with the 2B default, slower with 8B models. That is normal without a GPU.
...
| **Request timeout** | `LLM_REQUEST_TIMEOUT=300` in `.env`; monkey-patched `httpx` and `ollama.Client` defaults in `src/app.py`. | CPU inference for a full answer can take 60–180 seconds; default 30s timeouts would abort. |
